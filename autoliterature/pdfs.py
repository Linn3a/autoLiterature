...
class pdfDownload(object):
    ...
    def _get_available_scihub_urls(self):
        ...
        '''
        urls = []
        res = self.sess.get('https://lovescihub.wordpress.com/', proxies=self.sess.proxies)
    ...
    def fetch(self, url, auth=None):
        ...
        """
        try:
            r = self.sess.get(url, auth=auth, proxies=self.sess.proxies)
    ...
    def get_pdf_from_sci_hub(self, identifier, auth=None):
        ...
        for base_url in self._get_available_scihub_urls():
            r = self.sess.get(base_url + '/' + identifier, auth=auth, proxies=self.sess.proxies)
...
if __name__ == "__main__":
    ...
    pdf_download.set_proxy("127.0.0.1:1123")
